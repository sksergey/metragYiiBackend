...
<?php $form = ActiveForm::begin([
	...
	<div class="col-xs-12 col-sm-3 col-md-3 ">
		...
		<?= $form->field($model, 'type_object_id')->dropdownList(
    		TypeObject::find()->select(['name', 'type_object_id'])->where(['type_realty_id'=>'6'])->indexBy('type_object_id')->column())->label('Тип объекта'); ?>
	...
	<div class="col-xs-12 col-sm-3 col-md-3 ">
		...
		<?= $form->field($model, 'communication_id')->dropdownList(
    		Communication::find()->select(['name', 'communication_id'])->orderby('name')->indexBy('communication_id')->column(),['prompt'=>'Выберите ...'])->label('Коммуникации'); ?>
		<?= $form->field($model,'delivered')->textInput()->label('Был сдан'); ?>
	...
	<div class="col-xs-12 col-sm-3 col-md-3 ">
        <?= $form->field($model,'price_square_meter')->textInput()->label('Площадь общаяЦена за кв. м.'); ?>
		<?= $form->field($model,'total_area')->textInput()->label('Площадь общая'); ?>
		<?= $form->field($model,'total_area_house')->textInput()->label('Площадь общая'); ?>
        <?= $form->field($model,'housing')->checkbox()->label('Жилой фонд') ?>
        <?= $form->field($model,'documents')->checkbox()->label('Документы') ?>
        <?= $form->field($model,'topicality')->checkbox()->label('Актуальность') ?>
        <?= $form->field($model,'red_line')->checkbox()->label('Красная линия') ?>
        <?= $form->field($model,'phone_line')->checkbox()->label('Телефонная линия') ?>
        <?= $form->field($model,'detached_building')->checkbox()->label('Отдельно стоящее здание') ?>
        <?= $form->field($model,'rent')->checkbox()->label('Аренда') ?>
        <?= $form->field($model,'avtorampa')->checkbox()->label('Авторампа') ?>
        <?= $form->field($model,'infinite_period')->checkbox()->label('Без срока') ?>
		<?= $form->field($model,'separate_entrance')->checkbox()->label('Отдельный вход') ?>
...
<? $this->registerJs('
    $(document).ready(function () {
        ...
        function change_location(){
            var selectVal = $("input[name=\'Commercial[city_or_region]\']:checked").val();
            if (selectVal == \'1\') { 
                $("select[name=\'Commercial[locality_id]\']").removeAttr("disabled");
                $("select[name=\'Commercial[course_id]\']").removeAttr("disabled");
                $("select[name=\'Commercial[region_id]\']").removeAttr("disabled");
                $("select[name=\'Commercial[region_kharkiv_admin_id]\']").attr("disabled", true).find("[value=\'0\']").attr("selected", "selected");
                $("select[name=\'Commercial[region_kharkiv_id]\']").attr("disabled", true).find("[value=\'0\']").attr("selected", "selected");
                $("select[name=\'Commercial[metro_id]\']").attr("disabled", true).find("[value=\'0\']").attr("selected", "selected");
            } else {
                $("select[name=\'Commercial[region_kharkiv_admin_id]\']").removeAttr("disabled");
                $("select[name=\'Commercial[region_kharkiv_id]\']").removeAttr("disabled");
                $("select[name=\'Commercial[metro_id]\']").removeAttr("disabled");
                $("select[name=\'Commercial[locality_id]\']").attr("disabled", true).find("[value=\'0\']").attr("selected", "selected");
                $("select[name=\'Commercial[course_id]\']").attr("disabled", true).find("[value=\'0\']").attr("selected", "selected");
                $("select[name=\'Commercial[region_id]\']").attr("disabled", true).find("[value=\'0\']").attr("selected", "selected");
            }
        }

        $(\'#commercial-city_or_region\').change(change_location);
        ');
...
<?
 $this->registerJs('
    ...
    function save_phone() {
        ...
        $("input[name=\'Commercial[phone]\']").val(phone.join(","));
    }
 	')
?>
